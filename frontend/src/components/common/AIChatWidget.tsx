        ...
        <AnimatePresence>
          {!isOpen && (
            ...
            >
              ...
              <img src={logoUrl} alt="AI" className="w-8 h-8 object-contain drop-shadow-md brightness-0 invert" />
          ...
          >
              ...
              <div className="flex items-center gap-4 relative z-10">
                ...
                <div>
                  ...
                  <div className="flex items-center gap-1.5 mt-0.5">
                    ...
                    <p className="text-xs text-white/90 font-medium">Đang trực tuyến - Sẵn sàng tư vấn</p>
                  </div>
                </div>
              </div>
              <button 
              ...
              {messages.map((msg, idx) => (
                ...
                >
                  <div className={`w-10 h-10 rounded-full flex items-center justify-center shrink-0 shadow-sm ${
                    ...
                    {msg.role === 'user' ? <User size={18} /> : <img src={logoUrl} alt="AI" className="w-6 h-6 object-contain" />}
                  </div>
                  <div className={`max-w-[80%] rounded-2xl px-5 py-3.5 text-sm shadow-sm relative group ${
                    msg.role === 'user' 
                    ...
                    {msg.products && msg.products.length > 0 && (
                      <div className="mt-4 space-y-3">
                        {msg.products.map((p, i) => (
                          <div key={i} className="flex gap-3 p-2.5 rounded-xl bg-surface-container-lowest border border-outline-variant/50 items-center hover:bg-surface-container hover:border-primary/30 transition-all cursor-pointer shadow-sm group/product">
                            {p.image && <img src={p.image} alt={p.name} className="w-14 h-14 rounded-lg object-cover group-hover/product:scale-105 transition-transform" />}
                            <div className="flex-1 min-w-0">
                              <h4 className="font-bold text-xs truncate text-on-surface leading-tight mb-1">{p.name || 'Sản phẩm'}</h4>
                              <p className="text-primary font-black text-sm">{(p.price || 0).toLocaleString('vi-VN')}đ</p>
                            </div>
                            <span className="material-symbols-outlined text-primary opacity-0 group-hover/product:opacity-100 transition-opacity">chevron_right</span>
                          </div>
                        ))}
                      </div>
                    )}
                    <span className={`text-[9px] absolute -bottom-5 opacity-0 group-hover:opacity-100 transition-opacity font-medium ${msg.role === 'user' ? 'right-1 text-on-surface-variant/60' : 'left-1 text-on-surface-variant/60'}`}>
              ...
              {isLoading && (
                <div className="flex gap-3">
                  <div className="w-10 h-10 rounded-full bg-white text-primary border-2 border-primary/20 flex items-center justify-center shrink-0 shadow-sm">
                    <img src={logoUrl} alt="AI" className="w-6 h-6 object-contain opacity-50 animate-pulse" />
                  </div>
            ...
            <div className="p-4 bg-white border-t border-outline-variant/30 shrink-0 z-10 shadow-[0_-10px_20px_rgba(0,0,0,0.02)]">
              ...
              <div className="text-center mt-3">
                <span className="text-[10px] text-on-surface-variant/50 font-medium uppercase tracking-widest">Powered by Volitify AI ✨</span>
              </div>
            </div>
          </motion.div>
        )}
