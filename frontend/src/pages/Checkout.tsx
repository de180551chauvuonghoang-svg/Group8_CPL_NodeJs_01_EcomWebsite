...
function StepDot({ step, current, label }: { step: number; current: number; label: string }) {
  ...
  return (
    <div className="flex flex-col items-center gap-1.5">
      ...
      >
        {done ? <span className="material-symbols-outlined text-[18px]">check</span> : step}
      </div>
      <span
        className={`text-[11px] font-semibold ${active ? 'text-primary' : 'text-on-surface-variant'}`}
      >
        {label}
      </span>
    </div>
    ...
    >
      ...
      <div className="flex-1 min-w-0">
        <div className="flex items-center gap-2">
          <p
            className={`font-bold text-sm ${isSelected ? 'text-on-surface' : 'text-on-surface-variant'}`}
          >
            {title}
          </p>
          {badge && (
            <span
              ...
              style={{ background: 'rgba(0,74,198,0.15)', color: '#4a90e2' }}
            >
              {badge}
            </span>
          )}
...
export default function Checkout() {
  ...
  const { cartItems, couponCodes, discountAmount, clearCart, clearDiscount, refreshCartPrices } =
    useCart();
    ...
    refreshCartPrices()
      .then(({ pricesChanged, removedUnavailableItems }) => {
        if (removedUnavailableItems > 0) {
          setError(
            `${removedUnavailableItems} sản phẩm cũ hoặc không còn bán đã được xóa khỏi giỏ hàng. Vui lòng kiểm tra lại.`,
          );
        } else if (pricesChanged) {
          setError(
            'Giá sản phẩm đã thay đổi theo chương trình hiện tại. Vui lòng kiểm tra lại tổng tiền.',
          );
        }
      })
      .catch(() => {
        setError('Không thể kiểm tra giá mới nhất. Vui lòng tải lại trang trước khi thanh toán.');
      });
  }, [refreshCartPrices]);
  ...
  const handleSubmit = async () => {
    ...
    let refreshResult;
    try {
      refreshResult = await refreshCartPrices();
    } catch {
      setError('Không thể xác nhận giá sản phẩm mới nhất. Vui lòng thử lại.');
      setLoading(false);
      return;
    }

    if (refreshResult.removedUnavailableItems > 0) {
      setError(
        `${refreshResult.removedUnavailableItems} sản phẩm cũ hoặc không còn bán đã được xóa khỏi giỏ hàng. Vui lòng kiểm tra lại.`,
      );
      setLoading(false);
      return;
    }

    if (refreshResult.pricesChanged) {
      setError('Giá sản phẩm vừa thay đổi. Vui lòng kiểm tra lại đơn hàng trước khi thanh toán.');
      setLoading(false);
      return;
    }

    const refreshedItems = refreshResult.items;

    const confirmedSubtotal = refreshedItems.reduce(
      (sum, item) => sum + item.product.price * item.quantity,
      0,
    );
    const confirmedVat = Math.round(confirmedSubtotal * 0.1);
    const confirmedDiscount = Math.min(discountAmount, confirmedSubtotal);
    const confirmedTotal = Math.max(0, confirmedSubtotal + confirmedVat - confirmedDiscount);
    ...
    try {
      ...
      setCompletedOrderTotal(Number(res.pricing?.total ?? confirmedTotal));
      ...
      if (status === 403 && errorData?.code === 'OWN_SHOP_PURCHASE_NOT_ALLOWED') {
        setQrUrl('');
        setError(errorData.message || 'Bạn không thể mua sản phẩm của cửa hàng mình.');
        setLoading(false);
        return;
      }

      if (status === 409 && errorData?.code === 'PRICE_CHANGED') {
        clearDiscount();
        setQrUrl('');
        try {
          await refreshCartPrices();
        } catch {
          // Keep the backend message visible even if refreshing the cart fails.
        }
        setError(errorData.message || 'Giá sản phẩm đã thay đổi. Vui lòng kiểm tra lại đơn hàng.');
        setLoading(false);
        return;
      }
      ...
      if (
        ...
        clearDiscount();
        setQrUrl('');
        setError(errorData.message || 'Voucher không còn áp dụng được cho đơn hàng này.');
        setLoading(false);
        return;
      }

      if (['PRODUCT_UNAVAILABLE', 'INSUFFICIENT_STOCK'].includes(errorData?.code)) {
        setQrUrl('');
        try {
          await refreshCartPrices();
        } catch {
          // Keep the checkout error visible when refreshing also fails.
        }
        setError(errorData.message || 'Sản phẩm không còn đủ điều kiện để thanh toán.');
        setLoading(false);
        return;
      }

      setError(errorData?.message || err?.message || 'Đã xảy ra lỗi, vui lòng thử lại');
      setLoading(false);
    }
  };
      ...
      <div className="relative z-10 max-w-6xl mx-auto px-4 md:px-8">
        ...
        <div className="mb-10 text-center">
          ...
          <p className="text-on-surface-variant text-sm">
            Chỉ còn {step === 1 ? 'một' : 'một'} bước nữa thôi!
          </p>
        </div>
        ...
        <div className="flex items-center justify-center gap-0 mb-10">
          ...
          <div
            className={`w-20 h-0.5 mb-4 mx-1 rounded transition-all duration-500 ${step >= 2 ? 'bg-primary' : 'bg-white/10'}`}
          />
        ...
        <div className="grid grid-cols-1 lg:grid-cols-[1fr_380px] gap-8 items-start">
                ...
                >
                  ...
                  <div className="flex flex-col gap-3 mb-8">
                    <PaymentCard
                      method="qr"
                      selected={method}
                      onSelect={(m) => setMethod(m as 'cod' | 'qr')}
                      icon="qr_code"
                  ...
                  {qrUrl ? (
                    ...
                    <motion.button
                      ...
                      whileHover={
                        loading
                          ? {}
                          : { scale: 1.02, boxShadow: '0 20px 40px -8px rgba(0,74,198,0.45)' }
                      }
                    ...
                    >
                      {loading ? (
                        <>
                          <span className="material-symbols-outlined animate-spin">sync</span>Đang
                          xử lý...
                        </>
                      ) : method === 'qr' ? (
                        <>
                          <span className="material-symbols-outlined">qr_code</span>Thanh toán{' '}
                          {fmt(total)} qua VietQR
                        </>
                      ) : (
                        <>
                          <span className="material-symbols-outlined">check_circle</span>Đặt hàng
                          COD — {fmt(total)}
                        </>
                      )}
                    </motion.button>
          ...
          >
            ...
            <div className="mt-5 flex justify-center gap-6">
              {[
                { icon: 'verified_user', label: 'Bảo mật SSL' },
                { icon: 'cached', label: 'Đổi trả 30 ngày' },
              ].map(({ icon, label }) => (
        ...
        {showSuccessModal && (
          ...
          >
            ...
            >
              ...
              >
                <span
                  className="material-symbols-outlined text-green-500 text-[40px] font-bold"
              ...
              <p className="text-slate-500 text-sm mb-6 leading-relaxed">
                Cảm ơn bạn đã mua sắm tại <strong className="text-blue-600">Volitify</strong>. Đơn
                hàng của bạn đã được ghi nhận và đang trong quá trình xử lý.
              </p>
                ...
                <div className="bg-slate-50 rounded-2xl p-4 border border-slate-100 mb-6 text-left text-xs space-y-2">
                  <div className="flex justify-between">
                    ...
                    <span className="font-mono font-bold text-slate-700">
                      #{createdOrderId.toUpperCase()}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-500">Phương thức:</span>
                    <span className="font-bold text-slate-700">
                      {method === 'qr' ? 'Chuyển khoản VietQR' : 'Thanh toán COD'}
                    </span>
                  </div>
                  <div className="flex justify-between">
                    <span className="text-slate-500">Tổng thanh toán:</span>
                    <span className="font-black text-blue-600">{fmt(completedOrderTotal)}</span>
                  </div>
                </div>
              )}
