...
import { Loader2, MessageSquare, Package, Store } from 'lucide-react';
...
export default function ShopPublic() {
  ...
  useEffect(() => {
    ...
    setLoading(true);
    sellerService.getPublicShop(id)
      .then(response => {
  ...
  const handleChat = () => {
    ...
    if (shop?.user_id) {
      setLiveChatSeller(shop.user_id, {
        name: shop.shop_name,
  ...
  if (!shop) {
    return (
      <main className="mx-auto max-w-3xl px-6 py-24 text-center">
        ...
        <Link to="/products" className="mt-5 inline-flex rounded-xl bg-primary px-5 py-3 text-sm font-bold text-white">
          Xem sản phẩm
        </Link>
      </main>
    );
            ...
            <div className="flex flex-col gap-5 md:flex-row md:items-end md:justify-between">
              <div className="flex flex-col gap-4 sm:flex-row sm:items-end">
                ...
                <div className="min-w-0">
                  <h1 className="break-words text-3xl font-black text-on-surface">{shop.shop_name}</h1>
                  ...
                  <div className="mt-4 flex flex-wrap gap-2 text-xs font-bold text-on-surface-variant">
                    <span className="rounded-lg bg-surface-container px-3 py-1.5">{stats.total_products} sản phẩm</span>
                    <span className="rounded-lg bg-surface-container px-3 py-1.5">Đang hoạt động</span>
                  </div>
                </div>
              </div>
              ...
              <button
                type="button"
                onClick={handleChat}
                className="inline-flex h-11 shrink-0 items-center justify-center gap-2 rounded-xl border border-primary/30 px-4 text-sm font-bold text-primary transition hover:bg-primary/10"
              >
      ...
      <section className="mx-auto max-w-container-max px-6 py-10 md:px-margin-desktop">
        <div className="mb-5 flex flex-col gap-4 md:flex-row md:items-end md:justify-between">
          <div>
            ...
            <span className="text-sm text-on-surface-variant">{visibleProducts.length} sản phẩm đang hiển thị</span>
          </div>
              ...
              >
                ...
                <div className="p-3">
                  ...
                  <div className="mt-1 flex flex-wrap items-center gap-2">
                    ...
                    {product.originalPrice && product.originalPrice > product.price && (
                      <p className="text-xs font-semibold text-on-surface-variant line-through">{formatPrice(product.originalPrice)}</p>
                    )}
                  </div>
