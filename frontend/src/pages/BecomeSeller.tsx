...
import { BadgeCheck, Banknote, Image, Store, Phone, MapPin, FileText, Sparkles, ArrowRight, ShoppingBag, Clock3 } from 'lucide-react';
...
export default function BecomeSeller() {
  ...
  const handleSubmit = async (e: React.FormEvent) => {
    ...
    setLoading(true);
    try {
      // Không cập nhật role/token ở đây — shop tạo ra đang ở trạng thái
      // "chờ duyệt", chỉ khi admin duyệt thì tài khoản mới thực sự là seller.
      ...
      if (result?.sellerStatus === 'active') {
        setError('Bạn đã có một shop đang hoạt động, không thể đăng ký thêm.');
        return;
      }
  ...
  // Admin là tài khoản quản lý riêng, không được mở shop bán hàng
    ...
    return (
      <div className="min-h-screen bg-surface flex items-center justify-center p-4">
        <motion.div
          ...
          className="w-full max-w-md text-center bg-surface-container-lowest/80 backdrop-blur-xl rounded-3xl border border-outline-variant/30 shadow-2xl p-10"
        >
          <div className="inline-flex items-center justify-center w-20 h-20 bg-gradient-to-br from-primary to-secondary rounded-3xl mb-4 shadow-2xl shadow-primary/30">
            <Clock3 size={36} className="text-white" />
          </div>
          <h1 className="text-2xl font-black text-on-surface mb-2">Đơn Đang Chờ Duyệt</h1>
          <p className="text-on-surface-variant">
            Cảm ơn bạn đã đăng ký! Đơn mở shop của bạn đã được gửi tới admin.
            Bạn sẽ có thể truy cập Kênh Người Bán ngay sau khi đơn được duyệt.
          </p>
        </motion.div>
      </div>
    );
