          ...
          <div className="absolute top-3 left-3 flex flex-col gap-1.5">
            {product.isFlashSale &&
              ...
              product.originalPrice > product.price && (
                <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-error/90 text-on-error">
                  Sale
                </span>
              )}
            {isOutOfStock && (
            ...
            {!isOutOfStock && product.stock <= 5 && (
              <span className="text-[10px] font-bold px-2 py-0.5 rounded-full bg-warning/90 text-white">
                Sắp hết
              </span>
            )}
          </div>
...
export default function Products() {
  ...
  }, [selectedCat, searchVal, sortKey, setSearchParams]);
