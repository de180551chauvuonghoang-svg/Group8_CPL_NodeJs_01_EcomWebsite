...
export const createCODOrder = async (req, res, next) => {
  ...
  try {
    ...
    // A005: gửi email xác nhận đơn hàng (best-effort, không chặn response nếu lỗi)
    emailService
      .sendFromTemplate('order_confirmed', req.user.email, {
...
/**
 * GET /api/payments/orders/:orderId/items
 * Returns the line items for a specific order (authenticated, own orders only).
 * Includes a has_reviewed flag per item so the UI can show/hide the Review button.
 */
export const getOrderItems = async (req, res, next) => {
  try {
    ...
    const { recordset: orderCheck } = await pool.request()
      .input('orderId', orderId)
      .input('userId',  userId)
      .query(`SELECT id FROM Orders WHERE id = @orderId AND user_id = @userId`);
      ...
      .query(`
        SELECT oi.id,
               ...
               CASE WHEN EXISTS (
                 ...
                 WHERE r.product_id = pv.product_id AND r.user_id = @userId
...
export const cancelOrderAndRestoreStock = async (req, res, next) => {
  ...
  try {
    ...
    // A005: gửi email báo huỷ đơn (best-effort, không chặn response nếu lỗi)
    emailService
      .sendFromTemplate('order_cancelled', req.user.email, {
        customer_name: req.user.name,
        order_id: orderId
      })
      .catch(() => {});
